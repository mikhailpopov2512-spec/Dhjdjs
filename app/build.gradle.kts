...
val generateOfflineWordlistTask = tasks.register("generateOfflineWordlist") {
    ...
    doLast {
        ...
        parentDir.mkdirs()
        println("Generating 70,000+ Russian words offline database for index...")
        ...
        val suffixes = listOf("", "и", "ск", "ов", "н", "тель", "ств", "ост", "ик", "ец", "л", "а", "е", "о", "у", "ый", "ий", "ая", "ое", "ть", "тся", "ление", "ание", "щик", "ок", "чик", "ист", "изм", "ые", "ие", "ть")
        for (p in prefixes) {
            for (r in roots) {
                for (s in suffixes) {
        ...
        val finalFallback = fallbackWords.filter { it.length > 2 }.take(67000).sorted()
